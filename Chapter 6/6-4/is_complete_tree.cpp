...
/**
 * Checks whether the binary tree is a complete binary tree.
 *
 * The algorithm uses level-order traversal. Once a null child is seen,
 * all subsequent nodes must also be null for the tree to be complete.
 *
 * @return True if the tree is complete, false otherwise.
 */
template <class T>
bool binaryTree<T>::is_complete_tree() const {
    linkQueue<Node*> que;
    que.enQueue(root);

    bool null_seen = false;  // Flag to indicate if a null child has been seen

    while (!que.isEmpty()) {
        Node* curr = que.deQueue();

        if (curr == nullptr) {
            null_seen = true;  // After this point, all nodes must be null
        } else {
            if (null_seen) {
                return false;  // If a non-null node is seen after a null, the tree is not complete
            }
...
int main() {
    ...
    tree.levelOrder();
    cout << endl;
    tree.is_complete_tree() ? cout << "The tree is complete.\n" : cout << "The tree is not complete.\n";
